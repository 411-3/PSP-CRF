	...
	const int&					NUMITR)
{
	...
	while( numitr < NUMITR )
	{
		...
		*/
			//lh-1230
			int y1 = max(static_cast<double>(0), 		kseedsy[n]-offset);
			int y2 = min(static_cast<double>(m_height),	kseedsy[n]+offset);
			int x1 = max(static_cast<double>(0),		kseedsx[n]-offset);
	...
	const string&				path) 
{
	...
	printf("after init, enter to splitpath.\n");
	_splitpath(filename.c_str(), drive, dir, fname, extn);
	printf("in savesuperpixellabels.\n");
	printf("fname = %s\n",fname);
	printf("extn = %s\n",extn);
